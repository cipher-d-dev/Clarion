...
import { Button, Input } from "@clarion/ui";
  ...
  return (
    <div className="max-w-3xl space-y-5">
      <PageHeader
        ...
        breadcrumb={[{ label: "Complaints" }, { label: complaint.referenceNumber }]}
      ...
      {nextStatuses.length > 0 && (
        <Section title="Update Status" icon={<ArrowRight className="h-3 w-3" />}>
          <p className="text-xs text-slate-500 dark:text-slate-400 mb-3">
            Move this complaint to the next stage of resolution.
          </p>
          <div className="flex flex-wrap gap-2">
            {nextStatuses.map((s) => (
              <button
                ...
                className={cn(
                  "flex items-center gap-2 rounded-lg border px-3.5 py-2 text-[12.5px] font-semibold transition-all duration-150 cursor-pointer",
      ...
      <Section title="Internal Notes" icon={<Lock className="h-3 w-3" />}>
        <p className="text-xs text-slate-400 dark:text-slate-500 mb-4 -mt-1">
          Internal notes are not visible to the complaint submitter.
        </p>
      ...
      <ConfirmDialog
        ...
        description={`Move this complaint to "${confirmStatus?.replace(/_/g, " ")}"? This action will be logged in the timeline.`}
